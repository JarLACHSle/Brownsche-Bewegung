...
class Ball:
    ...
    def handle_collision(self, b2):
        ...
        if abstand <= self.radius + b2.radius and not (self.last_collision == b2 and b2.last_collision == self):
            ...
            count += 1
        abstand2 = self.distance(b2)
        if abstand2 > self.radius + b2.radius and count > 0:
            self.last_collision = None

    # idee: problem ist dass die teilchen nicht weit genug zurückgehen und dann wieder in die andere richtung fliegen -> count einführen?
    '''def repulse(self,b2):
        ab_x = b2.x - self.x
        ab_y = b2.y - self.y
